...
fn extract(kind: ArchiveKind, src: impl Read, dst: impl AsRef<Path>, pb: ProgressBar) -> io::Result<()> {
  use ArchiveKind::*;
...
async fn fetch_single_source(
  ...
  let pb = mp.insert(usize::MAX, ProgressBar::new(1));
  pb.set_style(
  ...
  match &file.location {
    SourceLocation::Http(url) => {
      ...
      if let Some((ar_kind, dir_name)) = ar_kind {
        ...
        pb.set_position(0);
        pb.set_message(format!("{}: extracting", file.file_name()));
        let pb2 = pb.clone();
        asyncify(move || {
  ...
  pb.finish_with_message(format!("{}: done", file.file_name()));
  Ok(())
}
